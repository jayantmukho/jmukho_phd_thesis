...
data_3c = sortrows(data_3c,12);
data_p1c1 = sortrows(data_p1c1,12);
data_p1c2 = sortrows(data_p1c2,12);
data_baseline = sortrows(data_baseline,12);

%%
alpha = data_baseline(:,12);
...
hold on
plot(aInterp,baseCLInterp(:,1),'k')%,expData(:,1),expData(:,2),'k.','MarkerSize',14)
...
fill(betweenX,betweenY,gray,'EdgeColor',gray);
hold on
plot(aInterp,baseCDInterp(:,1),'k')%,expData(:,1),expData(:,2),'k.','MarkerSize',14)
% errorbar(WT_Mach85.alpha,WT_Mach85.CL,2*WT_Mach85.CL_sig,'kx','LineWidth',2)
...
fill(betweenX,betweenY,gray,'EdgeColor',gray);
hold on
plot(aInterp,baseCmInterp(:,1),'k')%,expData(:,1),expData(:,2),'k.','MarkerSize',14)
% errorbar(WT_Mach85.alpha,WT_Mach85.CL,2*WT_Mach85.CL_sig,'kx','LineWidth',2)
...
figure(10); clf;
[betweenX,betweenY] = calculateExtents(minCD,maxCD,minCL,maxCL);
% betweenX= [minCD;flipud(maxCD)];
% betweenY= [minCL;flipud(maxCL)];
fill(betweenX,betweenY,gray,'EdgeColor',gray);
hold on
plot(baseCD,baseCL,'k')%,expData(:,1),expData(:,2),'k.','MarkerSize',14)
% errorbar(WT_Mach85.alpha,WT_Mach85.CL,2*WT_Mach85.CL_sig,'kx','LineWidth',2)
