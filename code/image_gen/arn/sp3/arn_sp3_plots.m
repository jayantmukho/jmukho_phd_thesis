...
for i=1:n
    ...
    elseif abs(simData(i,1)- 12) < .25 && abs(simData(i,2))<=1.6;
        x_base_12= [x_base_12; simData(i,:)];3  
...
%% 2c
simData = dlmread('3c/flow.dat','\t',3,0);
k=find(simData(:,12));
simData = simData(k,:);
simData = simData(1:4680,:);
...
opts = detectImportOptions('Tx_X_SP3_1ptn_Sym.dat');
expData = readmatrix('Tx_X_SP3_1ptn_Sym.dat',opts);
opts = detectImportOptions('Tx_X_SP3_1ptn_Sig.dat');
expErr = readmatrix('Tx_X_SP3_1ptn_Sig.dat',opts);
nExp = size(expData,1);
...
axis([.8,4.7,0,1.3])
saveas(gcf,'images/arn_sp3_vel_prof.png')
...
axis([1,15,0,1.4])
saveas(gcf,'images/arn_sp3_center_vel.png')
%% TKE Plots
gray=1/255*[200,200,200];
...
saveas(gcf,'images/arn_sp3_k_prof.png')