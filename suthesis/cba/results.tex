...
The results of the different databases are compared to the same single-, high-fidelity database that was used in the previous section.

An overarching trend across the four plots is that the variance in the metrics is increased when using a subset of the high-fidelity data. 
This is to be expected.
With less data to learn on, the uncertainty in the GP predictions increases.
This leads to greater variability in the database samples and, consequently, greater variance in the output metrics from the flight simulations. 
